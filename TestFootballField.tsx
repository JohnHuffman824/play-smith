...
export function FootballField() {
  ...
  const FIELD_WIDTH = 160; // Total width in feet
  const FIELD_LENGTH = 360; // 100 yards + 2 end zones (10 yards each) = 120 yards = 360 feet
  const HASH_SPACING = 3; // 3 feet apart (1 yard)
  const LEFT_HASH_POSITION = 60; // 60 feet from left edge
  const RIGHT_HASH_POSITION = 100; // 60 feet from right edge (160 - 60 = 100)
  ...
  // Loop through the field length creating markers every 3 feet (1 yard)
  for (let yards = 0; yards <= 120; yards++) {
    const yPosition = yards * HASH_SPACING * SCALE; // Convert yards to feet to pixels
    const isYardLine = yards % 5 === 0; // Every 5 yards
    ...
    if (isYardLine) {
      ...
      // Add yard number labels (skip end zones and show field numbers)
      if (yards >= 10 && yards <= 110) {
        const fieldYard =
          yards <= 60 ? yards - 10 : 120 - yards;
        if (
          fieldYard >= 0 &&
          fieldYard <= 50 &&
          fieldYard % 10 === 0
        ) {
          // Left side number - rotated -90 degrees, top at 15 feet from left edge
          const leftNumberX =
            (NUMBER_TOP_FROM_EDGE + NUMBER_HEIGHT / 2) * SCALE;
          fieldMarkers.push(
            <text
          ...
          // Right side number - rotated 90 degrees, top at 15 feet from right edge
          const rightNumberX =
            (FIELD_WIDTH -

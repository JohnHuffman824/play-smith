...
const host = 'playsmith-dev.c940uiy0ypml.us-east-2.rds.amazonaws.com';
...
try {
  ...
  console.log(`DATABASE_URL=postgres://${username}:${encodedPassword}@${host}:5432/${database}?sslmode=require`);
  ...
  if (error.message.includes('password authentication failed')) {
    ...
    console.error('  1. Check the master password in AWS Console → RDS → Databases → playsmith-dev');
    console.error('  2. If you lost the password, you can modify it:');
    console.error('     - Select playsmith-dev → Modify → New master password');
    console.error('  3. Make sure you\'re using the MASTER password, not any other password');

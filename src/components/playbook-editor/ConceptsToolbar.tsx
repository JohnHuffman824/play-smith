...
export type ConceptFilter = 'all' | 'routes' | 'motions' | 'modifiers' | 'formations' | 'groups'
...
export function ConceptsToolbar({ onNewConcept, activeFilter, onFilterChange }: ConceptsToolbarProps) {
	return (
		<div className="border-b border-border bg-card px-6 py-3">
			<div className="flex items-center gap-4 flex-1">
				<button onClick={onNewConcept} className={`${BUTTON_BASE} ${BUTTON_ACTIVE} flex items-center gap-2`}>
				...
				<div className="flex items-center gap-2">
					{FILTERS.map((filter) => (
						<button
							...
							className={`${BUTTON_BASE} ${activeFilter === filter.id ? BUTTON_ACTIVE : BUTTON_INACTIVE}`}
						>
							{filter.label}
						</button>
					))}
